...
export function defaultPrompt() {
  return "Create a hyper-realistic 5-second fashion video using all reference images and videos. The subject must remain perfectly still in the exact same pose during the entire clip, while only the camera rotates smoothly 360° around the person. Absolute fidelity to the main subject: preserve exactly the same face, hairstyle, body proportions, skin tone, outfit, fabric texture, stitching, accessories, shoes, and all front/back clothing details visible in the references. Solid pure green screen background only (#00ff00 chroma key), seamless and uniform, with no environment changes, props, reflections, shadows, gradients, or additional elements. Full-body centered framing, fixed distance, clean studio lighting, no distortion, no animation of clothing or body, no pose changes, no outfit changes, no extra people. Maintain realistic fabric behavior and accurate garment structure from every angle. Ultra-consistent identity and outfit continuity across all frames. The background must be easy to remove cleanly with chroma key.";
}
