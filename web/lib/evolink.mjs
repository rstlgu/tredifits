...
export function defaultPrompt() {
  return "Create a hyper-realistic 5-second fashion video using all reference images and videos. The subject must remain perfectly still in the exact same pose during the entire clip, while only the camera rotates smoothly 360° around the person. Absolute fidelity to the main subject: preserve exactly the same face, hairstyle, body proportions, skin tone, outfit, fabric texture, stitching, accessories, shoes, and all front/back clothing details visible in the references. Use a flat pure white background only: solid white (#ffffff), seamless, uniform, untextured, and evenly lit. The background must remain exactly #ffffff in every frame and every camera angle. No shadows on the background, no contact shadows, no cast shadows, no reflections, no gradients, no vignettes, no floor line, no horizon line, no studio wall, no props, no environment, and no additional elements. Full-body centered framing, fixed distance, clean studio lighting on the subject only, no distortion, no animation of clothing or body, no pose changes, no outfit changes, no extra people. Maintain realistic fabric behavior and accurate garment structure from every angle. Ultra-consistent identity and outfit continuity across all frames. The background must be easy to remove cleanly because it is a perfectly uniform white matte.";
}
